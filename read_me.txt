...
What I need from you (minimal) to finish the remaining pillars fast
I can draft the other 4 pillar SKILL.md files in the same quality/format immediately — but I need one choice:
	• Do you want each pillar to be:
A) questionnaire-first (works without PHC)
B) PHC-first (assumes PHC outputs exist, falls back to questions)
If you don’t answer, I’ll default to A, because it’s universally usable and easiest to eval well.